...
class SelfAttention(nn.Module): #单个head
    def __init__(self, m_V, m_h, d, n, batch_size,requires_grad=True,):
        ...
        self.W_O = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.d), int(self.m_V)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        self.W_K = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.m_h), int(self.d)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        ...
        self.W_V = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.m_V), int(self.d)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
...
class FFN(nn.Module):
    def __init__(self, d, r,requires_grad=True,):
        super(FFN, self).__init__()
        self.d = d
        self.r = r
        self.W_1 = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.r), int(self.d)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        self.W_2 = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.d), int(self.r)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        self.b_1 = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.r)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        self.b_2 = nn.Parameter(
            torch.tensor(
                torch.randn(int(self.d)), dtype=torch.float32, requires_grad=requires_grad
            )
        )
        self.relu = nn.ReLU()
    def forward(self, x):
    ...
    def forward(self, x): #x: (b, d0, n)->b
        ...
        if self.mode_c == 'max':
            ...
            output = torch.where(
                ...
                (self.K ) * torch.ones(self.batch_size),
                hat_c
                ) #b 
...
class Transformer(nn.Module):  
    def __init__(self, 
        ...
        self.classifier = classifier(d, K, mode_c, mode_E, lambda0, T, batch_size,n)
    def forward(self, x):
        x = self.fcnn(x)
        x = self.layers(x)
